...
namespace OpenSee
{
    ...
    public class OpenSeeLauncher : MonoBehaviour
    {
        ...
        public int ApplicationPersistence.AppSettings.Camera = -1;*/
        ...
        public bool StartTracker()
        {
            ...
            if (dynamicPort && !openSeeTarget.listening)
            {
                ...
                int port = TrackerSettings.Port + 1;
                for (int i = 0; i < inUse.Length; i++)
                {
                    ...
                    if (inUse[i].Port == port)
                    {
                        if (port <= TrackerSettings.Port + 500 && port <= IPEndPoint.MaxPort)
                        {
                            port++;
                            continue;
                        }
            ...
            if (videoPath != "" && File.Exists(videoPath))
                ...
                arguments.Add(ApplicationPersistence.AppSettings.Camera.ToString());

            arguments.Add("--model");
            arguments.Add(TrackerSettings.Quality.ToString());
