...
public class OpenSeeVRMDriver : MonoBehaviour
{
    ...
    public class OpenSeeBlendShapeProxy
    {
    ...
    {
#if WINDOWS_BUILD
        if (ApplicationPersistence.AppSettings.LipSyncProvider == LipSyncProvider.OculusLipSync)
        {
    ...
    void UpdateGaze()
    {
        ...
        {
            switch (ApplicationPersistence.AppSettings.Tracker)
            {
                ...
                case Tracker.Mediapipe:
                    lookLeftRight = (openSeeData.leftGazeVec.y + openSeeData.rightGazeVec.y) / 2.0f * 0.035f;
                    lookUpDown = (openSeeData.leftGazeVec.x + openSeeData.rightGazeVec.x) / 2.0f * 0.035f;
                    break;
            }
    ...
    public void InitializeLipSync()
    {
        if (ApplicationPersistence.AppSettings.LipSyncProvider == LipSyncProvider.OculusLipSync)
        {
            ...
            lastMic = ApplicationPersistence.AppSettings.Microphone;
                    ...
                    if (totalLen < 1024 * channels)
                    {
                        ...
                        lock (this)
                        {
                            if (context != 0)
                            {
                                ...
                                //else
                                OVRLipSync.ProcessFrame(context, partialAudio, frame, channels == 2);
                            }
                            else
                            {
    ...
    void RunUpdates()
    {
        ...
        {
            doMouthTracking = false;
        }
#else
        bool doMouthTracking = false;
#endif
        if (doMouthTracking)
            ApplyMouthShape();
